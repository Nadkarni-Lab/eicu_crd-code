...
-- -
-- Reference:
-- - https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iv/concepts/medication/norepinephrine.sql
-- History:
...
-- WHERE LOWER(rx.drugname) LIKE '%norepinephrine%'
...
DROP TABLE IF EXISTS aims_eicu_crd_derived.norepinephrine;
CREATE TABLE aims_eicu_crd_derived.norepinephrine AS
WITH rx1 AS (
    SELECT ie.uniquepid,
           ...
           CASE
               ...
               WHEN lower(rx.drugname) LIKE '%(mg/hr)%'
                   THEN CASE
                            WHEN rx.patientweight IS NOT NULL AND rx.patientweight > 10
                                THEN round(SAFE_CAST(rx.drugrate AS NUMERIC) * 1000 / rx.patientweight / 60, 3)
                            ELSE round(SAFE_CAST(rx.drugrate AS NUMERIC) * 1000 / 80 / 60, 3) END
    ...
    FROM aims_eicu_crd_icu.icustays AS ie
             INNER JOIN `physionet-data.eicu_crd.infusiondrug` AS rx
                        ON ie.patientunitstayid = rx.patientunitstayid
    WHERE LOWER(rx.drugname) LIKE '%norepinephrine%'
      AND rx.drugname NOT IN ('Norepinephrine (mg/kg/min)', 'Norepinephrine (mg/hr)', 'Norepinephrine (mg/min)', 'Norepinephrine (units/min)', 'Norepinephrine ()')
      AND REGEXP_CONTAINS(rx.drugrate, '^[0-9]*[.]?[0-9]+$')
      AND rx.drugrate <> ''
      AND rx.drugrate <> '.'
...
SELECT uniquepid,
       ...
       CASE
           WHEN end_ho IS NULL
               THEN LEAST(start_ho + 60 * 3, outtime_ho)
           ELSE LEAST(start_ho + 60 * 3, end_ho)
           END AS end_ho,
       CASE
           WHEN end_io IS NULL
               THEN LEAST(start_io + 60 * 3, outtime_io)
           ELSE LEAST(start_io + 60 * 3, end_io)
           END AS end_io,
       drugname,
       drugrate,
       infusionrate,
       ...
       CASE
           WHEN endtime IS NULL
               THEN LEAST(starttime + 60 * 3, outtime)
           ELSE LEAST(starttime + 60 * 3, endtime)
           END AS endtime,
       vaso_rate,
       -- endtim
FROM rx2
