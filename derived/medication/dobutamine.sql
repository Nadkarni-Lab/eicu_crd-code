...
-- -
-- Reference:
-- - https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iv/concepts/medication/dobutamine.sql
-- History:
...
-- WHERE LOWER(rx.drugname) LIKE '%dobutamin%'
...
DROP TABLE IF EXISTS aims_eicu_crd_derived.dobutamine;
CREATE TABLE aims_eicu_crd_derived.dobutamine AS
WITH rx1 AS (
    SELECT ie.uniquepid,
    ...
    FROM aims_eicu_crd_icu.icustays AS ie
             INNER JOIN `physionet-data.eicu_crd.infusiondrug` AS rx
                        ON ie.patientunitstayid = rx.patientunitstayid
    WHERE LOWER(rx.drugname) LIKE '%dobutamin%'
      AND rx.drugname NOT IN ('Dobutamine ()')
      AND REGEXP_CONTAINS(rx.drugrate, '^[0-9]*[.]?[0-9]+$')
      AND rx.drugrate <> ''
      AND rx.drugrate <> '.'
...
SELECT uniquepid,
       ...
       CASE
           WHEN end_io IS NULL
               THEN LEAST(start_io + 60 * 12, outtime_io)
       ...
       CASE
           WHEN endtime IS NULL
               THEN LEAST(starttime + 60 * 12, outtime)
           ELSE LEAST(starttime + 60 * 12, endtime)
           END AS endtime,
       vaso_rate,
       -- endtim
FROM rx2
