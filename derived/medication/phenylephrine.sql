...
-- -
-- Reference:
-- - https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iv/concepts/medication/phenylephrine.sql
-- History:
...
-- WHERE LOWER(rx.drugname) LIKE '%phenylephrine%'
...
DROP TABLE IF EXISTS aims_eicu_crd_derived.phenylephrine;
CREATE TABLE aims_eicu_crd_derived.phenylephrine AS
WITH rx1 AS (
    SELECT ie.uniquepid,
           ...
           CASE
               ...
               WHEN lower(rx.drugname) LIKE '%Volume (ml) Phenylephrine ()%'
                   THEN CASE
                            WHEN rx.patientweight IS NOT NULL AND rx.patientweight > 10
                                THEN round(
                                    SAFE_CAST(rx.drugrate AS NUMERIC) * 1000 /
    ...
    FROM aims_eicu_crd_icu.icustays AS ie
             INNER JOIN `physionet-data.eicu_crd.infusiondrug` AS rx
                        ON ie.patientunitstayid = rx.patientunitstayid
    WHERE LOWER(rx.drugname) LIKE '%phenylephrine%'
      AND rx.drugname NOT IN ('Phenylephrine (mg/kg/min)', 'Phenylephrine (mg/hr)', 'Phenylephrine ()')
      AND REGEXP_CONTAINS(rx.drugrate, '^[0-9]*[.]?[0-9]+$')
      AND rx.drugrate <> ''
      AND rx.drugrate <> '.'
...
SELECT uniquepid,
       ...
       CASE
           WHEN end_ho IS NULL
               THEN LEAST(start_ho + 60 * 2, outtime_ho)
           ELSE LEAST(start_ho + 60 * 12, end_ho)
           END AS end_ho,
       CASE
           WHEN end_io IS NULL
               THEN LEAST(start_io + 60 * 2, outtime_io)
           ELSE LEAST(start_io + 60 * 12, end_io)
           END AS end_io,
       drugname,
       drugrate,
       infusionrate,
       ...
       CASE
           WHEN endtime IS NULL
               THEN LEAST(starttime + 60 * 2, outtime)
           ELSE LEAST(starttime + 60 * 2, endtime)
           END AS endtime,
       vaso_rate,
       -- endtim
FROM rx2
